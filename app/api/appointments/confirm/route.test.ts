...
function makeRequest(body: object, secret = 'test-webhook-secret') {
  return new NextRequest('http://localhost/api/appointments/confirm', {
    method: 'POST',
...
describe('POST /api/appointments/confirm', () => {
  ...
  it('returns 401 with wrong secret', async () => {
    const res = await POST(makeRequest({ appointmentId: 'apt-1' }, 'wrong-secret'));
  ...
  it('marks the appointment as confirmed and notifies via WhatsApp', async () => {
    ...
    expect(mockUpdate).toHaveBeenCalledWith(expect.objectContaining({ reminder_status: 'confirmed' }));
  ...
  it('still returns success:true if WhatsApp notification fails', async () => {
    mockGetOutboundAppointment.mockResolvedValue(appointment);
    mockSendWhatsApp.mockRejectedValue(new Error('bridge down'));
